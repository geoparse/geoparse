...
class Karta:
    ...
    def _base_map() -> folium.Map:
        ...
        karta.add_child(MeasureControl(position="topleft"))
        ...
        for data in data_list:
            ...
            if len(gdf) <= main_layer_max_records:
                ...
                if not cluster and not heatmap:
                    ...
                    print(
                        f"Dataset size ({len(gdf):,} records) exceeds threshold (main_layer_max_records={main_layer_max_records:,})",
                        flush=True,
                    )
...
class Karta2:
    ...
    def _create_plp_layer(
        ...
        if popup_dict:
            ...
            # Include useful columns for tooltips
...
class GeomUtils:
    """
    A utility class for performing common geometric operations on Shapely geometry objects
    and for converting various data formats into geometries.
...
class GamlaKarta:
    ...
    def _base_map(sw: list, ne: list) -> folium.Map:
        ...
        >>> karta = GamlaKarta._base_map(sw, ne)
    ...
    def _select_color(col: int | float | str, head: int | None = None, tail: int | None = None) -> str:
        ...
        Examples
        --------
        >>> GamlaKarta._select_color("Category1")
        '#e6194b'  # Red color from the palette

        >>> GamlaKarta._select_color(5)
        '#3cb44b'  # Green color from the palette

        >>> GamlaKarta._select_color("Example", head=0, tail=3)
        '#e12348'  # Bright Red from the palette
        """
        # Predefined color palette
        palette = [
    ...
    def _add_point(
        ...
        >>> GamlaKarta._add_point(row, karta, "color")
        """
        try:
        ...
        # Determine color if column is specified
            ...
            color = GamlaKarta._select_color(row[color])
    ...
    def _add_line(
        ...
        >>> GamlaKarta._add_line(row, karta, color="red", popup_dict={"Name": "road_name"})
        """
        coordinates = [(coord[1], coord[0]) for coord in row.geometry.coords]

        # Handle color selection
        color = GamlaKarta._select_color(row[color]) if color in row.index else color
    ...
    def _add_poly(
        ...
        >>> GamlaKarta._add_poly(row, karta, "fill_color", line_width=2)
        """
        # Determine fill color if specified column is present
        fill_color = GamlaKarta._select_color(row[fill_color]) if fill_color in row.index else fill_color
    ...
    def plp(
        ...
        if cells:
            ...
            gdf = gpd.GeoDataFrame({"id": cells, "res": res, "geometry": geoms}, crs="EPSG:4326")
            karta = GamlaKarta.plp(gdf, poly_popup={"ID": "id", "Resolution": "res"})
            return karta
        ...
        if osm_ways:
            ...
            if isinstance(gdf.geometry[0], LineString):
                karta = GamlaKarta.plp(gdf, line_popup={"way_id": "way_id"}, line_color="red")
            else:
                karta = GamlaKarta.plp(
                    gdf,
                    geohash_res=geohash_res,
                    s2_res=s2_res,
                    h3_res=h3_res,
        ...
        sw = [minlat, minlon]  # South West (bottom left corner)
        ne = [maxlat, maxlon]  # North East (top right corner)
        karta = GamlaKarta._base_map(sw, ne)  # Initialize folium map with the bounding box
