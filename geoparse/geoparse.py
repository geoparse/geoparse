...
class GeomUtils:
    ...
    bearing(self: LineString) -> tuple[int, int, str, str]
...
class DistanceCalculator:
    """
    A utility class for calculating distances between two points on the Earth's surface.

    This class provides methods for computing distances using the Haversine formula and the
    Vincenty formula. The Haversine formula calculates the great-circle distance assuming
    a spherical Earth, while the Vincenty formula accounts for the Earth's ellipsoidal shape
    and provides higher accuracy.
    ...
    vincenty(lat1: float, lon1: float, lat2: float, lon2: float) -> float
        Calculates the geodesic distance between two points using the Vincenty formula.

    Notes
    -----
    - The Haversine formula is simpler and faster but assumes a spherical Earth, which may
      introduce small errors for long distances or near-polar regions.
    - The Vincenty formula is more accurate but computationally intensive and may fail to
      converge in edge cases, such as antipodal points or points on the same meridian.

    Examples
    --------
    >>> calculator = DistanceCalculator()

    >>> # Example for Haversine formula
    >>> distance_haversine = calculator.haversine(52.2296756, 21.0122287, 41.8919300, 12.5113300)
    >>> print(distance_haversine)
    1319743.483

    >>> # Example for Vincenty formula
    >>> distance_vincenty = calculator.vincenty(52.2296756, 21.0122287, 41.8919300, 12.5113300)
    ...
    def vincenty(self, lat1: float, lon1: float, lat2: float, lon2: float) -> float:
        """
...
class SpatialOps:
    ...
    def flatten_3d(self, geom: gpd.GeoSeries) -> List[Union[Polygon, MultiPolygon]]:
        """
    ...
    def line_to_points(self, row: gpd.GeoSeries) -> gpd.GeoDataFrame:
        """
    ...
    def intersection(self, gdf1: gpd.GeoDataFrame, gdf2: gpd.GeoDataFrame, poly_id: Optional[str] = None) -> gpd.GeoDataFrame:
        """
    ...
    def quick_intersection(
        self, gdf1: gpd.GeoDataFrame, gdf2: gpd.GeoDataFrame, poly_id: Optional[str] = None
    ) -> gpd.GeoDataFrame:
        """
    ...
    def poverlay(
        self, gdf1: gpd.GeoDataFrame, gdf2: gpd.GeoDataFrame, how: str = "intersection", keep_geom_type: bool = False
    ) -> gpd.GeoDataFrame:
        """
...
class GeocodingUtils:
    """
    A utility class for performing geocoding and reverse geocoding operations using the Google Geocoding API.

    This class provides methods to convert addresses or zip codes into geographic coordinates (latitude and longitude)
    and to reverse geocode coordinates into postal codes. It leverages the Google Geocoding API for these operations.

    Methods
    -------
    geocoding_google(address_or_zipcode: str, api_key: str) -> pd.Series
        Converts an address or zip code into geographic coordinates (latitude and longitude) using the Google Geocoding API.

    reverse_geocoding_google(lat: float, lon: float, api_key: str) -> str
        Converts geographic coordinates (latitude and longitude) into a postal code using the Google Geocoding API.

    Notes
    -----
    - Both methods require a valid Google Maps API key with access to the Geocoding API.
    - Ensure that the API key has sufficient quota and is enabled for the Geocoding API in the Google Cloud Console.
    - Handle exceptions and `None` values appropriately, as the API may fail to find results or exceed request limits.

    Examples
    --------
    >>> geocoding_utils = GeocodingUtils()

    >>> # Example for geocoding_google
    >>> coordinates = geocoding_utils.geocoding_google("1600 Amphitheatre Parkway, Mountain View, CA", "your_api_key")
    >>> print(coordinates)
    lat    37.4224764
    lon   -122.0842499
    dtype: float64

    >>> # Example for reverse_geocoding_google
    >>> postal_code = geocoding_utils.reverse_geocoding_google(37.4224764, -122.0842499, "your_api_key")
    >>> print(postal_code)
    '94043'
    """
    ...
    def geocoding_google(self, address_or_zipcode: str, api_key: str) -> pd.Series:
        """
    ...
    def reverse_geocoding_google(self, lat: float, lon: float, api_key: str) -> str:
        """
...
class PolylineDecoder:
    @staticmethod  # PolylineDecoder.decode is a static method since it doesn't rely on instance-specific data.
...
class MapMatcher:
    def __init__(self, url: str):
        """
        Initializes the MapMatcher with the Meili service URL.
    ...
    def map_matching(self, df: pd.DataFrame, cost: str, url: str, format: str = "osrm") -> Optional[dict]:
        """
        ...
        response = requests.post(self.url, data=meili_request_body, headers={"Content-type": "application/json"})
