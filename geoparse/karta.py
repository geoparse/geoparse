...
def plp(  # plp: points, lines, polygons
    ...
    if cells:
        res, geoms = indexer.cellpoly(cells, cell_type=cell_type)
    ...
    if geohash_res > 0:  # inner=False doesn't work if compact=True
        ...
        cells, _ = indexer.ppolycell(cdf, cell_type="geohash", res=geohash_res, compact=compact)
        res, geoms = indexer.cellpoly(cells, cell_type="geohash")
    ...
    if s2_res > -1:
        ...
        cells, _ = indexer.ppolycell(cdf, cell_type="s2", res=s2_res, compact=compact)
        res, geoms = indexer.cellpoly(cells, cell_type="s2")
        cdf = gpd.GeoDataFrame({"id": cells, "res": res, "geometry": geoms}, crs="EPSG:4326")
    ...
    if h3_res > -1:
        ...
        cells, _ = indexer.ppolycell(cdf, cell_type="h3", res=h3_res, compact=compact)
        res, geoms = indexer.cellpoly(cells, cell_type="h3")
        cdf = gpd.GeoDataFrame({"id": cells, "res": res, "geometry": geoms}, crs="EPSG:4326")
